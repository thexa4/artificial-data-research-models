...
def run_eval(run_dir, checkpoint_dir, hparams):
  ...
  for checkpoint_path in slim.evaluation.checkpoints_iterator(
      checkpoint_dir, FLAGS.eval_interval_secs, timeout=1):
    with tf.Graph().as_default():
      ...
      #hparams.batch_size = target_dataset.num_samples
      ...
      def save_image(tensor):
        ...
        def save_recursive(tensor, curstep, source, label = "unknown"):
          global count
          count = count + 1
          fullfile = run_dir + "/results/converted/" + str(curstep) + "." + FLAGS.source_split_name + "." + str(count) + "." + str(label) + ".png"
          fullfile_source = run_dir + "/results/original/" + str(curstep) + "." + FLAGS.source_split_name + "." + str(count) + "." + str(label) + ".png"
...
def create_metrics(end_points, source_labels, target_labels, hparams):
  ...
  ##########################################################################
  if 'quaternion' in list(source_labels.keys()):
    params = {}
