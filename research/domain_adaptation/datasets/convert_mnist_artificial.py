...
r"""Converts MNIST-Artificial data to TFRecords of TF-Example protos.
...
def _convert_dataset(split_name, filenames, filename_to_class_id, dataset_dir):
  ...
  if split_name in ['train', 'valid']:
    png_directory = os.path.join(dataset_dir, 'mnist_artificial', 'mnist_artificial_train')
  elif split_name == 'test':
    png_directory = os.path.join(dataset_dir, 'mnist_artificial', 'mnist_artificial_test')
...
def _get_output_filename(dataset_dir, split_name):
  ...
  """
  return '%s/mnist_artificial_%s.tfrecord' % (dataset_dir, split_name)
...
def run(dataset_dir):
  ...
  train_filename = _get_output_filename(dataset_dir, 'mnist_artificial_train')
  testing_filename = _get_output_filename(dataset_dir, 'mnist_artificial_test')
  ...
  train_validation_filenames = _get_filenames(
      os.path.join(dataset_dir, 'mnist_artificial', 'mnist_artificial_train'))
  test_filenames = _get_filenames(
      os.path.join(dataset_dir, 'mnist_artificial', 'mnist_artificial_test'))
  ...
  test_filenames_to_class_ids = _extract_labels(
      os.path.join(dataset_dir, 'mnist_artificial', 'mnist_artificial_test_labels.txt'))
  ...
  print('\nFinished converting the MNIST-Artificial dataset!')
